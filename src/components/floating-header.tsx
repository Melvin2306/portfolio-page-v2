...
interface FloatingHeaderProps {
  ...
  goBackLink?: string; // Assuming goBackLink is optional
  children?: React.ReactNode; // Assuming children is optional
}
...
export const FloatingHeader = memo(function FloatingHeader({
  ...
  useEffect(() => {
    ...
    const onScroll = (e: Event) => {
      // Assert e.target as an HTMLElement to access scrollTop
      ...
      const opacity = Math.min(
        Math.max(
          // Convert the result back to a number if .toFixed() or similar was used
          Number(
            (
              (scrollY -
